                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="type" id="type" class="form-select @error('type') is-invalid @enderror" required>
                            ...
                            @foreach($types as $key => $value)
                                <option value="{{ $key }}" {{ old('type', $menu->type) == $key ? 'selected' : '' }}>{{ $value }}</option>
                            @endforeach
                        </select>
                        @error('type')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="parent_id" id="parent_id" class="form-select @error('parent_id') is-invalid @enderror">
                            ...
                            @foreach($parentMenus as $parentMenu)
                                <option value="{{ $parentMenu->menu_id }}" {{ old('parent_id', $menu->parent_id) == $parentMenu->menu_id ? 'selected' : '' }}>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="is_active" id="is_active" class="form-select @error('is_active') is-invalid @enderror" required>
                            <option value="1" {{ old('is_active', $menu->is_active) == 1 ? 'selected' : '' }}>Hoạt động</option>
                            <option value="0" {{ old('is_active', $menu->is_active) == 0 ? 'selected' : '' }}>Tạm tắt</option>
                        </select>
                        @error('is_active')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="datetime-local" id="created_at"
                            class="form-control bg-light"
                            value="{{ $menu->created_at->format('Y-m-d\TH:i') }}"
                            readonly>
                    </div>
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="datetime-local" id="updated_at"
                            class="form-control bg-light"
                            value="{{ $menu->updated_at->format('Y-m-d\TH:i') }}"
                            readonly>
                    </div>
                </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#type').on('change', function() {
            ...
            var currentParentId = "{{ $menu->parent_id }}";
            var currentMenuId = "{{ $menu->menu_id }}";
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $.each(data, function(index, item) {
                            // Không cho chọn chính menu đang edit làm parent
                            if (item.menu_id != currentMenuId) {
                                var selected = (item.menu_id == currentParentId) ? 'selected' : '';
                                parentSelect.append('<option value="' + item.menu_id + '" ' + selected + '>' + item.name + '</option>');
                            }
                        });
                    }
                });
